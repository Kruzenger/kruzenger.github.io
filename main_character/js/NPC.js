...
function NPC_what_to_do() {
	if (npc_angry == ON) {
		...
		if (npc_talked == ON) {
			if ((npc_position - Xposition) < -300 && length_of_walk <= 6500) {
				npc_moving = ON;
				npc_run();
			}
			else if ((npc_position - Xposition) > 300 && length_of_walk <= 6500) {
				npc_moving = ON;
				npc_run();
			}
...
function npc_moving_run(){
	if (npc_side == RIGHT && npc_moving == ON && length_of_walk <= 6500) {
		npc_position += 2;
		npc.style.left = npc_position + 'px';
	}
	else if(npc_side == LEFT && npc_moving == ON  && length_of_walk <= 6500){
		npc_position -= 2;
		npc.style.left = npc_position + 'px';
	}
}