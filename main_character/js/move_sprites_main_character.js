		...
		if(cadr_hit_width_right < 1850){
			...
			if (cadr_hit_width_right < 2050) {
				...
				if (enemy_side3 == RIGHT && (enemy_position3 - Xposition) >= 0 && (enemy_position3 - Xposition) <= 120) {		// all numbers is in px
					enemy_atack3 = ON;
					enemy_health_bar3.value -= DAMAGE;
					enemy_pain3();
				}
...
function where_am_I(){
	...
	else if (length_of_walk >= 400 && length_of_walk <= 600 && frase_N2 == OFF) {
		all_clear();
		frase_N2 = ON;
		teach_fighting();
	}
	else if(length_of_walk >= 1600 && length_of_walk <= 1700 && frase_N3 == OFF && Health.value < 100){
		all_clear();
		frase_N3 = ON;
		teach_healing();
	}
...
function use_healing(){
	if((healing_position - Xposition) <= 50 && (healing_position - Xposition) >= -50){
		...
		setTimeout(function(){ Health.value += 50;}, 500);
		setTimeout(function(){
			healing_position = 4200;
			healing.style.left = healing_position + "px";
