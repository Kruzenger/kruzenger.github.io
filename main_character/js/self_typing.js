...
function number_of_damage_moob(damage, position){
	if (hren > 0) {
		delete_all_numbers();
	}
	var number_of_damage = document.createElement('div');
	number_of_damage.id = 'number_of_damage_on_moob';
	if (damage == 40){
		number_of_damage.innerHTML = '-40';
	} 
	else if (damage == 25) {
		number_of_damage.innerHTML = '-25';
	}
	number_of_damage.style.left = position + 'px';
	game.appendChild(number_of_damage);
	hren++;
	var inter = setInterval(function(){
		if (just_call == OFF) {
			just_call = ON;
			object = document.getElementById('number_of_damage_on_moob');
			setTimeout(function(){move = ON; number_of_damage.remove();}, 1150);
		}
		else if (move == OFF) {
			object.style.opacity = '0';
			number_top -= 1;
			object.style.top = number_top + 'px';
		}
		else{
			move = OFF;
			just_call = OFF;
